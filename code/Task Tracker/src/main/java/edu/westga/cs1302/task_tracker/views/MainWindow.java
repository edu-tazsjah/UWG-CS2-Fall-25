...
public class MainWindow {
	...
	/** Displays the current SubTask in a popup window
	 * 
	 * @precondition selectedSubTask != null
	 * @postcondition none
	 * 
	 */
	@FXML
	public void viewSelectedSubtask() {
		try {
			Task selectedSubTask = this.subtasks.getSelectionModel().getSelectedItem();
			if (selectedSubTask == null) {
				throw new IllegalArgumentException("Please select a subtask to display.");
			}

			String details = "Name: " + selectedSubTask.getName() + System.lineSeparator()
			+ "Description: " + selectedSubTask.getDescription() + System.lineSeparator()
			+ "Priority: " + selectedSubTask.getPriority();

			Alert alert = new Alert(Alert.AlertType.INFORMATION);
			alert.setTitle("Subtask Details");
			alert.setHeaderText("Selected Subtask Information");
